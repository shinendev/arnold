...
export const config = {
  ...
  maxTweetsPerDay: Math.min(
    10,
    Math.max(3, parseInt(process.env.ARNOLD_MAX_TWEETS_PER_DAY || '5', 10) || 5),
  ),
  /** Replies count toward maxTweetsPerDay but are capped extra-tight. */
  maxReplyTweetsPerDay: Math.min(
    5,
    Math.max(0, parseInt(process.env.ARNOLD_MAX_REPLY_TWEETS_PER_DAY || '2', 10) || 2),
  ),
  ...
  processUpdateIntervalMs: parseInt(
    process.env.ARNOLD_PROCESS_UPDATE_INTERVAL_MS || String(48 * 60 * 60 * 1000),
    10,
  ),
  mentionCheckIntervalMs: parseInt(
    process.env.ARNOLD_MENTION_CHECK_INTERVAL_MS || String(4 * 60 * 1000),
    10,
  ),
  insightCheckIntervalMs: parseInt(
    process.env.ARNOLD_INSIGHT_CHECK_INTERVAL_MS || String(10 * 60 * 1000),
    10,
  ),
  /** Only tweet insights at or above this confidence (stricter = fewer tweets). */
  minInsightConfidenceForTweet: parseFloat(
    process.env.ARNOLD_MIN_INSIGHT_CONFIDENCE || '0.75',
  ),
