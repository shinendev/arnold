...
class Consolidator {
    ...
    async consolidate() {
        ...
        for (const [factA, factB] of contradictions) {
            ...
            const insight = await this.insightStore.addInsight(insightText, [factA.id, factB.id], 0.6, 'consolidation');
            entries.push({
                id: (0, uuid_1.v4)(),
                timestamp: Date.now(),
                phase: 'consolidate',
    ...
    findContradictions(facts) {
        ...
        for (let i = 0; i < factsWithEmbeddings.length; i++) {
            for (let j = i + 1; j < factsWithEmbeddings.length; j++) {
                ...
                const sim = (0, embeddings_1.cosineSimilarity)(a.embedding, b.embedding);
