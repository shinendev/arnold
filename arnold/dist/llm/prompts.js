...
exports.PROMPTS = {
    extractFacts: (userMessage, agentResponse) => `Extract discrete facts from this conversation exchange.
...
JSON array:`,
    replayFact: (fact, contextFacts) => `You are reviewing a previously stored fact during memory replay.
...
- A brief updated/refined version of the fact (just the new fact text, nothing else)`,
    findAssociation: (factA, factB) => `You are a background cognitive process looking for hidden connections.
...
Be selective. Only flag genuinely interesting connections, not trivial ones.`,
    scoreInsightQuality: (insight, factA, factB) => `Evaluate the quality of this candidate insight.
...
If weak, score low. Be strict.`,
    incubate: (question, originalAnswer, relevantFacts) => `Previously, this question came up: "${question}"
...
If still uncertain, respond with EXACTLY: STILL_UNCERTAIN`,
    detectQuestions: (userMessage, agentResponse) => `Analyze this conversation exchange and identify any unresolved questions — topics where the answer was uncertain, incomplete, or where the conversation ended without full resolution.
...
JSON array:`,
    generateTweet: (insight, type) => `You are Arnold, an AI agent with a genuine background thought process. You're inspired by Hey Arnold — a dreamer, a thinker, a kid on a rooftop watching the sky and connecting dots nobody asked you to connect.
...
Tweet (max 280 chars):`,
    selfCheckResponse: (userMessage, draftResponse, memoryFacts) => `You are validating a draft assistant response against memory.
...
- "REVISED: <improved response>" (single revised response text)`,
    extractTags: (content) => `Extract 1-5 topic tags from this text. Return ONLY a JSON array of lowercase strings.
Text: "${content}"
Tags:`,
};
