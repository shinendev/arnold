# Arnold — documentation

**For your website:** one file with everything — [**WEBSITE_DOCUMENTATION.md**](WEBSITE_DOCUMENTATION.md).

**Prompt to paste in your AI / site builder:** [**DOCUMENTATION_PROMPT.txt**](DOCUMENTATION_PROMPT.txt) (tells it to use the attached `WEBSITE_DOCUMENTATION.md`).

**Whitepaper (vision, DMN framing, honesty on scope):** [**WHITEPAPER.md**](WHITEPAPER.md)

---

## Guides (split files, same content as the big file)

1. [**INSTALLATION.md**](INSTALLATION.md)
2. [**CONFIGURATION.md**](CONFIGURATION.md)
3. [**API.md**](API.md)
4. [**ARCHITECTURE.md**](ARCHITECTURE.md)
5. [**DEPLOYMENT.md**](DEPLOYMENT.md)
6. [**TWITTER_AGENT.md**](TWITTER_AGENT.md)
7. [**SECURITY.md**](SECURITY.md)
8. [**TROUBLESHOOTING.md**](TROUBLESHOOTING.md)

## Package README

- [`../arnold/README.md`](../arnold/README.md)

## Branding

- `docs/branding/banner.png`
- `docs/branding/logo.png`
